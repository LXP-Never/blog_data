...
def _rnn_layer(inputs, rnn_cell, rnn_hidden_size, layer_id, is_batch_norm, is_bidirectional, training):
    ...
    if is_bidirectional:
        outputs, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=fw_cell, cell_bw=bw_cell,
                                                     inputs=inputs, dtype=tf.float32, swap_memory=True)
        rnn_outputs = tf.concat(outputs, -1)
    else:
        rnn_outputs, _ = tf.nn.dynamic_rnn(fw_cell, inputs, dtype=tf.float32, swap_memory=True)
...
class DeepSpeech2(object):
    ...
    def __call__(self, inputs, training):
        # Two cnn layers.
        inputs = _conv_bn_layer(inputs, padding=(20, 5), filters=_conv_filters,
                                kernel_size=(41, 11), strides=(2, 2), layer_id=1, training=training)

        inputs = _conv_bn_layer(inputs, padding=(10, 5), filters=_conv_filters,
                                kernel_size=(21, 11), strides=(2, 1), layer_id=2, training=training)
